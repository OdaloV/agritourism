...
export default function Visitoreg() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ SUCCESS - Store email for verification
      localStorage.setItem("pendingVerificationEmail", formData.email);
      
      // ✅ Redirect to verification page using window.location for reliable navigation
      window.location.href = "/auth/verify-email";
      
    } catch (error) {
      console.error("Registration error:", error);
      setErrors((prev) => ({ ...prev, email: "Registration failed. Please check your connection and try again." }));
      setIsSubmitting(false);
    }
  };
